...
I've been trying to figure this out for years. Previous notes include [Using LD_PRELOAD to run any version of SQLite with Python](https://til.assahbismark.com/sqlite/ld-preload) (Linux only), and [Building a specific version of SQLite with pysqlite on macOS/Linux](https://til.assahbismark.com/sqlite/build-specific-sqlite-pysqlite-macos) and [Using pysqlite3 on macOS](https://til.assahbismark.com/sqlite/pysqlite3-on-macos) (both using the `pysqlite3` package). But the dream was always to find a way to let me easily run a different SQLite version with the `sqlite3` module from the Python standard library directly on my Mac.

[Alex Garcia](https://til.assahbismark.com/sqlite/pysqlite3-on-macos) helped me find the solution I've been looking for this morning.
